...
local function dump_log_to_disk(t)
    ...
    local file_src = CCFileUtils:sharedFileUtils():getWritablePath();
	local all_msg = ""
    for i=1,#t do
		local level, timestamp, src, msg = unpack(t[i])
		local msg = level .. "  " .. msg .. " " .. src .. " " .. timestamp
		all_msg = all_msg .. msg .. "\n"
    end
	print(file_src)
    local modname = "sok"
    local name = "lualog"
	local ext_name = ".log"
	local file_name = modname .. name .. os.time() .. ext_name
	local f = assert(io.open(file_src .. "/" .. file_name, 'wb+'))
	f:write(all_msg)
	f:close()
end
...
function logger:get_log_message(level, timestamp, src, ...)
	...
	local ext_name = self.logger_ext_name
    local timestamp = log_timestamp(timestamp)
    return name, ext_name, modname, level, timestamp,msg, src, tags
end
...
function logger:dump()
    ...
    -- for i= head,tail do
    --     log_message_list[#log_message_list + 1] = {self:get_log_message(unpack(self.dump_list[i]))}
    -- end
    dump_log_to_disk(self.dump_list)
...
function logger:log(level, ...)
	...
	local timedate = os.date("%Y-%m-%d %H:%M:%S",timestamp)
	print(level .. "  " .. ... .. " " .. timedate)
    ...
    if level >= self.log_level then  -- 高于记录级别, 缓存
	...
	if(self.dump_level ~= logger.NOLOG and self.dump_list.tail >= self.dump_list.len) then
		--print("dump start")
		self:dump()
	end
end
...
function logger:new()
    -- private field
    local obj = {}
    obj.default_module_name = "sok"
    obj.logger_name = "lualog"
	obj.logger_ext_name = ".log"
    obj.module_name = "sok"
    obj.to_screen = true
    obj.log_level = logger.DEBUG
    obj.log_src = false
    obj.dump_level = logger.DEBUG
    obj.dump_num = 5
